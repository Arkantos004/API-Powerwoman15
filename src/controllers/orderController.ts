...
export const createOrder = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const userResult = await query('SELECT email, full_name, phone, address FROM users WHERE id = $1', [userId]);
    const user = userResult.rows[0];

    // Obtener detalles de los items para la factura
    const itemsDetailsResult = await query(
      `SELECT oi.product_id, p.name, oi.quantity, oi.price_cop, oi.subtotal_cop
       FROM order_items oi
    ...
    const invoiceItems = itemsDetailsResult.rows.map((item: any) => ({
      name: item.name,
