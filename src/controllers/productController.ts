...
export const createProduct = async (req: Request, res: Response): Promise<void> => {
  try {
    const { name, description, category, price_cop, image_url, stock_quantity } =
    ...
    const result = await query(
      `INSERT INTO products (name, description, category, price_cop, image_url, stock_quantity) 
       VALUES ($1, $2, $3, $4, $5, $6) 
       RETURNING *`,
      [name, description || null, category, price_cop, image_url || null, stock_quantity || 0]
    );
  ...
  try {
    ...
    const result = await query(
      `UPDATE products 
       ...
       WHERE id = $7 
       RETURNING *`,
      [name, description, category, price_cop, stock_quantity, is_available, id]
    );
